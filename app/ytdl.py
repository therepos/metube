...
class DownloadInfo:
    def __init__(
        ...
        self.custom_filename = custom_filename or None
        self.custom_artist = custom_artist or None
        self.custom_album = custom_album or None
        self.use_default_cover = use_default_cover
        self.custom_cover_data = custom_cover_data or None
...
class DownloadQueue:
    ...
    def _post_download_cleanup(self, download):
        if download.info.status != 'finished':
            ...
            # Apply metadata processing for audio downloads if any custom metadata was provided
            self._apply_metadata(download)
        download.close()
    ...
    def _apply_metadata(self, download):
        """Apply custom metadata (tags, cover, rename) to finished audio downloads."""
        info = download.info
        custom_filename = getattr(info, 'custom_filename', None)
        custom_artist = getattr(info, 'custom_artist', None)
        custom_album = getattr(info, 'custom_album', None)
        use_default_cover = getattr(info, 'use_default_cover', True)
        custom_cover_data = getattr(info, 'custom_cover_data', None)

        has_metadata = any([custom_filename, custom_artist, custom_album])
        ...
        if info.quality != 'audio' and info.format not in AUDIO_FORMATS:
            return

        if not getattr(info, 'filename', None):
            return

        dldirectory, _ = self.__calc_download_path(info.quality, info.format, info.folder)
        if not dldirectory:
            return

        filepath = os.path.join(dldirectory, info.filename)
        if not os.path.isfile(filepath):
            log.warning(f"Cannot apply metadata - file not found: {filepath}")
            return

        try:
            new_filepath = process_metadata(
                filepath=filepath,
            ...
            # Update info with new filename and file size
            if new_filepath != filepath:
                info.filename = os.path.relpath(new_filepath, dldirectory)
            info.size = os.path.getsize(new_filepath) if os.path.exists(new_filepath) else info.size
            if custom_filename:
                info.title = custom_filename
        except Exception as e:
            log.error(f"Metadata processing failed for {filepath}: {e}")
            ...
            for index, etr in enumerate(entries, start=1):
                ...
                results.append(
                    await self.__add_entry(
                        ...
                        subtitle_mode,
                        already,
                        # Don't pass metadata for playlist items — only for single videos
                    )
                )
