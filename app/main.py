...
async def add(request):
    ...
    custom_artist = post.get('custom_artist')
    custom_album = post.get('custom_album')
    use_default_cover = post.get('use_default_cover', True)
    custom_cover_data = post.get('custom_cover_data')

    if custom_filename is not None:
        custom_filename = str(custom_filename).strip()
        if custom_filename and ('..' in custom_filename or '/' in custom_filename or '\\' in custom_filename or '\x00' in custom_filename):
            raise web.HTTPBadRequest(reason='custom_filename must not contain path separators, "..", or null bytes')
