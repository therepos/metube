...
export class App implements AfterViewInit, OnInit {
  ...
  customCoverData: string | null = null;
  customCoverPreview: string | null = null;
  ...
  addDownload(
    ...
    customCoverData = customCoverData !== undefined ? customCoverData : this.customCoverData
    ...
    this.downloads.add(url, quality, format, folder, customNamePrefix, playlistItemLimit, autoStart, splitByChapters, chapterTemplate, subtitleFormat, subtitleLanguage, subtitleMode, customFilename, customArtist, customAlbum, useDefaultCover, customCoverData ?? undefined).subscribe((status: Status) => {
  ...
  retryDownload(key: string, download: Download) {
    this.addDownload(
      ...
      download.custom_artist,
      download.custom_album,
      download.use_default_cover,
      download.custom_cover_data,
    );
  ...
  startBatchImport(): void {
    ...
    const processNext = () => {
      ...
      this.downloads.add(url, this.quality, this.format, this.folder, this.customNamePrefix,
        ...
        this.customFilename, this.customArtist, this.customAlbum,
        this.useDefaultCover, this.customCoverData ?? undefined)
        .subscribe({
          next: (status: Status) => {
            if (status.status === 'error') {
  ...
  onCoverImageSelected(event: Event) {
    ...
    if (input.files && input.files[0]) {
      ...
      reader.onload = () => {
        ...
        this.customCoverData = result.split(',')[1];
        this.customCoverPreview = result;
      };
  ...
  clearCustomCover() {
    this.customCoverData = null;
    this.customCoverPreview = null;
  }
